...
export default function DinerDashboard(props: Props) {
  ...
  function formatRole(role: { role: Role; objectId?: string }) {
    if (role.role === Role.Franchisee) return `Franchisee on ${role.objectId}`;
    return role.role;
  }
   ...
   async function updateUser() {
    ...
    const saved = await pizzaService.updateUser(updated);
    props.setUser(saved);
    setShowDialog(false);
  }
  ...
  if (!user) {
    return (
      <View title="Your pizza kitchen">
        <div className="text-neutral-100 p-6">Loading your profile…</div>
      </View>
    );
  }
  ...
  return (
    <View title="Your pizza kitchen">
      <div className="text-start py-8 px-4 sm:px-6 lg:px-8">
        ...
        {/* Edit button */}
        <div className="mt-3">
          <Button title="Edit" className="w-16 p-0" onPress={() => setShowDialog(true)} />
        </div>
        ...
        <div className="my-4 text-lg text-orange-200 text-start grid grid-cols-5 gap-2">
          ...
          <div className="col-span-4">
            {user.roles?.map((role, i) => (
              <span key={i}>{i === 0 ? '' : ', '} {formatRole(role)}</span>
            ))}
          </div>
        </div>
        ...
        {orders?.length === 0 && (
          <div className="text-neutral-100">
            ...
            <Link className="text-orange-400 underline font-semibold" to="/menu">Buy one</Link> now!
          </div>
        )}
                    ...
                    <div className="overflow-hidden">
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="uppercase text-neutral-100 bg-slate-400 border-b-2 border-gray-500">
                          <tr>
                            <th className="px-6 py-3 text-start text-xs sm:text-sm font-medium">ID</th>
                            <th className="px-6 py-3 text-start text-xs sm:text-sm font-medium">Price</th>
                            <th className="px-6 py-3 text-start text-xs sm:text-sm font-medium">Date</th>
                          </tr>
                        </thead>
                          ...
                          {orders.map((order, index) => (
                            <tr key={index} className="hover:bg-gray-100">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-start text-xs sm:text-sm text-gray-800">
                                {order.items.reduce((a, c) => a + (c.price ?? 0), 0).toLocaleString()} ₿
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap text-start text-xs sm:text-sm text-gray-800">
                                {new Date((order as any).date ?? (order as any).createdAt ?? Date.now()).toLocaleString()}
                              </td>
                            </tr>
                          ))}
                        </tbody>
      ...
      {/* Edit dialog (React-controlled) */}
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="dialog-title"
        id="hs-jwt-modal"
        className={`${showDialog ? '' : 'hidden pointer-events-none'} size-full fixed top-10 start-0 z-[80] overflow-x-hidden overflow-y-auto`}
      >
        <div className="sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)]">
          <div className="w-full flex flex-col bg-white border shadow-sm rounded-xl">
            ...
            <div className="p-4 overflow-y-scroll max-h-60">
              <div className="my-2 text-start grid grid-cols-5 gap-2 items-center">
